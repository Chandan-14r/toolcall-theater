...
def verify_live():
    print("Starting Node live agent server...")
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Wait for agent execution to succeed (takes ~1.5s due to retries delay)
            print("Waiting for agent execution to complete successfully...")
            # We wait for status text to become 'succeeded'
            status_locator = page.locator("#status-text")
            status_locator.wait_for(timeout=5000)
            time.sleep(1.5) # Let rendering finish

            # Capture screenshot showing the success timeline and retries
            ...
            assert events_count >= 3, "Timeline should show step attempts"
